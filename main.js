// Funzioni per gestire le domande
...
// ----------------------------
// Gestione monete e categorie
// ----------------------------

// Monete iniziali se non presenti
if (!localStorage.getItem("tokens")) {
  localStorage.setItem("tokens", 100); // 100 monete iniziali
}
...
if (lastLogin !== today) {
  ...
  alert("Bonus giornaliero! Hai ricevuto 100 monete 🎉");
}

// Categorie sbloccate di default
let unlocked = JSON.parse(localStorage.getItem("unlockedCategories")) || [];
if (!unlocked.includes("cultura")) {
  unlocked.push("cultura");
  localStorage.setItem("unlockedCategories", JSON.stringify(unlocked));
}

function updateTokenDisplay() {
  const tokens = localStorage.getItem("tokens") || 0;
  document.getElementById("token-display").innerText = "Gettoni: " + tokens;
}
updateTokenDisplay();
